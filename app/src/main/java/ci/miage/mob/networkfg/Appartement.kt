...
import ci.miage.mob.networkfg.R

class Appartement(context: Context, attrs: AttributeSet) : View(context, attrs) {

    var graph: MutableList<Graphs> = mutableListOf()
    val noeuds: MutableList<Noeuds> = mutableListOf()
    val arcs: MutableList<Connexion> = mutableListOf()

    private val connectedArcs: MutableList<Connexion> = mutableListOf()
    ...
    private var longPressedArc: Connexion? = null
    ...
    fun setGraphdt(graph: MutableList<Graphs>) {
        this.graph = graph
    ...
    private fun showModifyDialogArc(arcSelect: Connexion) {
    ...
    private fun showCustomDialogForArc(arcset: Connexion) {
    ...
    private fun calculateDistanceToArc(point: PointF, arc: Connexion): Float {
        ...
        // Calcul de la distance entre le point donné et le point de la connexion le plus proche
    ...
    private fun findNearestArc(point: PointF): Connexion? {
        var nearestArc: Connexion? = null
            ...
            MotionEvent.ACTION_UP -> {
                if (addConnectionMode && startPoint != null && endPoint != null) {
                    ...
                    nearestNode?.let { node ->
                        ...
                        startNode?.let { start ->
                            val newArc = Connexion(
                                idConnexion = arcs.size + 1, // ou utilisez un mécanisme pour générer un nouvel ID unique
    ...
    private fun removeArc(arc: Connexion) {
        arcs.remove(arc)
        invalidate()
    }
